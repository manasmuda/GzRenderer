...
int Application5::Render() 
{
	...
	GzRender renderers[AAKERNEL_SIZE];

	for (int i = 0; i < AAKERNEL_SIZE; i++)
	{
		renderers[i].CopyRenderer(m_pRender);
		renderers[i].xOffset = AAFilter[i][0];
		renderers[i].yOffset = AAFilter[i][1];

		status |= renderers[i].GzDefault();
	}
	...
	*/ 
	...
	int size = m_pRender->xres * m_pRender->yres;
	for (int j = 0; j < size; j++)
	{
		m_pRender->framebuffer[j] = 0;
	}

	
	//renderers[i].GzFlushDisplay2FrameBuffer();
	for (int x = 0; x < m_pRender->xres; x++)
	{
		for (int y = 0; y < m_pRender->yres; y++)
		{
			GzIntensity mr = 0, mg = 0, mb = 0, ma = 0;
			GzDepth mz = 0;
			for (int i = 0; i < AAKERNEL_SIZE; i++)
			{
				int index = renderers[i].ARRAY(x, y);
				GzPixel pixel = renderers[i].pixelbuffer[index];
				float weight = AAFilter[i][2];
				mr += weight * (pixel.red);
				mg += weight * (pixel.green);
				mb += weight * (pixel.blue);
				ma += weight * (pixel.alpha);
				mz += weight * (pixel.z);
			}
			m_pRender->GzPut(x, y, mr, mg, mb, ma, mz);
		}
	}
